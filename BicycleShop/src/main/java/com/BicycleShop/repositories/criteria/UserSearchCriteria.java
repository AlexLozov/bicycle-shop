...
public class UserSearchCriteria implements Specification<User> {
    ...
    public Predicate toPredicate(
        ...
        if(Objects.nonNull(request.getUsername())){
            predicates.add(criteriaBuilder.like(root.get(User.USERNAME_FIELD), "%" + request.getUsername() + "%"));
        }

        if(Objects.nonNull(request.getEmail())){
            predicates.add(criteriaBuilder.like(root.get(User.EMAIL_FIELD), "%" + request.getEmail() + "%"));
        }
    ...
    private void sort(Root<User> root,
